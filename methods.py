# There are some unnecessary features for further use
import numpy as np
# p is full 81bp DNA string
# v1,v2,v3,v4 is A,C,G,T respectively
...
# Convert Character into numeric value
def scl(p):
    if p == 'A':
        return 0
    elif p == 'C':
...
def string_matching(str1, str2, threshold):
    ...
    for i in range(len(str1)-len(str2)):
        ...
        for j in range(0, len(str2)):
            if(str1[i+j] == str2[j]):
                cnt += 1
        if (cnt>counter) and (cnt > threshold):
            counter = cnt
...
def cluster_interchange(cluster0, class0, cluster1, class1):
    temp_cluster0 = np.concatenate([cluster0[class0==1], cluster1[class1==0]])
    temp_class0 = np.concatenate([class0[class0 == 1], class1[class1 == 0]])

    temp_cluster1 = np.concatenate([cluster1[class1==1], cluster0[class0==0]])
    temp_class1 = np.concatenate([class1[class1==1], class0[class0==0]])

    return temp_cluster0, temp_class0, temp_cluster1, temp_class1